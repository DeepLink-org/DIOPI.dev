...
PYBIND11_MODULE(export_functions, m) {
    ...
    m.def("diopiAmpForeachNonFiniteCheckAndUnscaleInp", [](diopiContextHandle_t ctx, py::list& scaled_grads, int64_t num_scaled_grads, diopiTensorHandle_t found_inf, diopiConstTensorHandle_t inv_scale) {
        if (diopiAmpForeachNonFiniteCheckAndUnscaleInp) {
            py::gil_scoped_release no_gil;
            std::vector<diopiTensorHandle_t> scaled_gradsV(num_scaled_grads);
            for (int i = 0; i < num_scaled_grads; ++i)
                scaled_gradsV[i] = scaled_grads[i].cast<PtrWrapper<diopiTensor>>().get();
            auto scaled_gradsDIOPI = scaled_gradsV.data();
            diopiError_t ret = diopiAmpForeachNonFiniteCheckAndUnscaleInp(ctx, scaled_gradsDIOPI, num_scaled_grads, found_inf, inv_scale);
    ...
    m.def("diopiAmpUpdateScaleInp", [](diopiContextHandle_t ctx, diopiTensorHandle_t current_scale, diopiTensorHandle_t growth_tracker, diopiConstTensorHandle_t found_inf, double scale_growth_factor, double scale_backoff_factor, int32_t growth_interval) {
        if (diopiAmpUpdateScaleInp) {
            py::gil_scoped_release no_gil;
    
            diopiError_t ret = diopiAmpUpdateScaleInp(ctx, current_scale, growth_tracker, found_inf, scale_growth_factor, scale_backoff_factor, growth_interval);
