...
at::Tensor& index_put_aicpu(at::Tensor& result, at::Tensor& self, std::vector<at::Tensor> all_defined_indices, at::SmallVector<int64_t, N> masks,
    ...
    if (!npu_utils::check_match(&self)) {
        std::cout << "aicpu self需要转连续" << std::endl;
...
at::Tensor& index_put_aicore(at::Tensor& self, std::vector<at::Tensor> indices_expand, at::SmallVector<int64_t, N> masks,
    ...
    if (!npu_utils::check_match(&self)) {
        std::cout << "aicore self需要转连续" << std::endl;
        at::Tensor contiguous_self = npu_utils::format_contiguous(self);
...
at::Tensor& indexPut(at::Tensor& self, const c10::List<c10::optional<at::Tensor>>& indices, const at::Tensor& value, const bool accumulate, const bool unsafe) {
    ...
    TORCH_CHECK(op_plugin::AdvanceIndex::is_expandable_to(value.sizes(), info.src.sizes()),
                ...
                info.src.sizes());
    // 将expand后的index加入到all_defined_indices
    // mask作为标记符，0标记空
    for (c10::optional<at::Tensor> index_opt : indices_expand) {
        // 包含at::Tensor
        if (index_opt.has_value()) {
            const auto& index = *index_opt;
            // 再判断at::Tensor是否为empty tensor
            if (index.defined()) {
    ...
    // std::cout << "self_copy: " << self_copy << std::endl;
    return self;
}
