...
cast_strategy = {
    ...
    "CastFloatOnly": {"float64": "float32"},
    "LogicOp": {"int64": "int32", "float64": "int32"},
}
...
def get_functions_support(source_dir: str) -> Tuple[dict, dict]:
    ...
    for idx, row in enumerate(content):
        if row.startswith(" * type"):
            ...
            if "type" in row:
                row_new = row
            elif not content[idx + 1].startswith(" */") and not content[
                idx + 1
            ].startswith(" * @"):
        ...
        if "sa" in funcs_info[func].keys():
            ...
            for i in from_func_info["ins"]:
                ...
                if "grad_" + i in funcs_info[func]["outs"]:
                    funcs_info[func]["outs"]["grad_" + i] = from_func_info[
                        "ins"
                    ][i]
            for out in from_func_info["outs"]:
                ...
                if "grad_" + out in funcs_info[func]["ins"]:
                    funcs_info[func]["ins"]["grad_" + out] = from_func_info[
                        "outs"
                    ][out]
...
def deal_dtype(
    op_name: str, dtype_config: str, func_infos: dict, tensor_name: str = None
):
    strategy = {}
    ...
    for s in cast_strategy:
        ...
        for d in strategy:
            if (
                d not in cast_strategy[s].keys()
                or cast_strategy[s][d] != strategy[d]
            ):
                in_strategy = False
                break
        if in_strategy:
    ...
    for info in config:
        ...
        if op_name == "common_config":
            ...
            if "layout" in op_cfg.keys():
                common_layout = (
                    [op_cfg["layout"]]
                    if isinstance(op_cfg["layout"], str)
                    else op_cfg["layout"]
                )
            ...
            if "tensor_dtype" in op_cfg.keys():
                for tensor in op_cfg["tensor_dtype"]:
                    assert (
                        tensor in funcs_info[op_name]["ins"].keys()
                        or tensor in funcs_info[op_name]["outs"].keys()
                    )
                    tensor_cast = deal_dtype(
                        op_name,
                        op_cfg["tensor_dtype"][tensor],
                        funcs_info,
                        tensor,
                    )
                    op_tensor[tensor] = {}
            ...
            if "contiguous" in op_cfg.keys():
                contiguous_tensor = (
                    op_cfg["contiguous"].replace(" ", "").split(",")
                )
            ...
            for tensor in list(funcs_info[op_name]["ins"].keys()) + list(
                funcs_info[op_name]["outs"].keys()
            ):
                if tensor not in op_tensor.keys():
                    op_tensor[tensor] = {}
                if "cast" not in op_tensor[tensor]:
                    op_tensor[tensor]["cast"] = (
                        op_cast if op_cast else common_cast
                    )
                if "contiguous" not in op_tensor[tensor]:
                    op_tensor[tensor]["contiguous"] = (
                        True if common_contiguous else False
                    )
                if "layout" not in op_tensor[tensor]:
                    op_tensor[tensor]["layout"] = (
                        op_layouts if len(op_layouts) else common_layout
                    )
...
def autogen_cast_strategy():
    ...
    break;".format(
                    from_dtype=str_to_diopi_dtype[dtype],
                    to_dtype=str_to_diopi_dtype[
                        cast_strategy[strategy][dtype]
                    ],
                )
            )
        cast_code.append(
            OT.cast_strategy_template.substitute(
...
def autogen_op_adaptor(
    ...
    layout = (
        op_configs["Common"]["layout"] if "Common" in op_configs.keys() else []
    )
    for func in func_infos:
        ...
        if (
            (
                ...
                and "Common" not in op_configs.keys()
            )
            or len(list(func_infos[func].keys())) == 1
            or op_name in exclude_ops
        ):
            call_args = [
            ...
            cast_outs = []
            new_input = []
            for tensor in list(func_infos[func]["ins"].keys()) + list(
                func_infos[func]["outs"].keys()
            ):
                tensor_info = (
                    op_config["tensor"][tensor] if op_config else None
                )
                cast_method = tensor_info["cast"] if tensor_info else cast
                memory_format = (
                    tensor_info["layout"] if tensor_info else layout
                )
                format_str = memory_format_to_str(memory_format)
                ins = func_infos[func]["ins"]
                if tensor in ins:
                    if (
                        "ins_vector" in func_infos[func].keys()
                        and tensor in func_infos[func]["ins_vector"].keys()
                    ):
...
"""
                        ...
                        new_input.append(
                            new_ins_vector_template.substitute(
                                env=dict(
                                    input=tensor,
                                    newinput="new" + tensor.capitalize(),
                                    num=func_infos[func]["ins_vector"][tensor],
                                    cast=", " + cast_method
                                    if cast_method
                                    else "",
                                    memory_format=format_str,
                                )
                            )
                        )
                    else:
                        ...
                        cast_impl = "castImpl<diopiConstTensorHandle_t{cast}>(ctx, {tensor}, &{new_tensor}{memory_format});".format(
                            cast=", " + cast_method if cast_method else "",
                            memory_format=format_str,
                            tensor=tensor,
                            new_tensor=new_in,
                        )
                ...
                if tensor in outs:
                    cast_impl = "DiopiTensorWrapper<{cast}> {tensor}Wrapper(ctx, {tensor}{memory_format}, {inp});".format(
                        ...
                        if (
                            "Inp" in op_name
                            or tensor in inp_config.get(op_name, [])
                        )
                        else "false",
                    )
                    cast_outs.append(cast_impl)
            new_input.append(
                "diopiConstTensorHandle_t " + ", ".join(new_ins) + ";"
            ) if len(new_ins) else ""
            call_args = []
            for arg in func_infos[func]["call_args"]:
                ...
                if (
                    "ins_vector" in func_infos[func].keys()
                    and name in func_infos[func]["ins_vector"].keys()
                ):
...
def get_composite_funcs_declaration(
    funcs_decl_raw: dict, funcs_info: dict, impl_funcs: dict, op_configs: dict
) -> dict:
    composite_funcs_decl: dict = {}
    for func in funcs_info.keys():
        if func not in impl_funcs and op_configs.get(func, {}).get(
            "supportComposite"
        ):
